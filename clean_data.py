...
os.environ["POLARS_MAX_THREADS"] = "14"  # Change this based on your number of system threads (logical processors in Windows) (cores*threads per core in Linux)
...
task1_business_review_path = Path(__file__).parent / "data" / "task1_business_review.parquet"
task1_business_tip_path = Path(__file__).parent / "data" / "task1_business_tip.parquet"
...
# Examine data
print(business_df.filter(pl.col("review_count").ge(55)).group_by(pl.col("state")).len().sort(pl.col("len")).collect(streaming=True))
